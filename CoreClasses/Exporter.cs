...
namespace CloudCoinCore
{
    public class Exporter
    {
        ...
        public bool writeJSONFile(int m1, int m5, int m25, int m100, int m250, String tag)
        {
            ...
            json = json + "\t] " + Environment.NewLine;
            json += "}";
            String filename = (this.fileSystem.ExportFolder + Path.DirectorySeparatorChar + totalSaved + ".EOS Stealth." + tag + ".stack");
            if (File.Exists(filename))
            {
                // tack on a random number if a file already exists with the same tag
                Random rnd = new Random();
                int tagrand = rnd.Next(999);
                filename = (this.fileSystem.ExportFolder + Path.DirectorySeparatorChar + totalSaved + ".EOS Stealth." + tag + tagrand + ".stack");
            }//end if file exists
        ...
        public bool writeJSONFile(int m1, int m5, int m25, int m100, int m250, String tag, int mode = 0, string backupDir = "")
        {
            ...
            if (mode == 1)
            {
                filename = (backupDir + Path.DirectorySeparatorChar + totalSaved + ".EOS Stealth." + tag + ".stack");
            }
            if (File.Exists(filename))
            {
                // tack on a random number if a file already exists with the same tag
